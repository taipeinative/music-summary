...
class XMLSource:
    ...
    songs: pd.DataFrame = field(default_factory = lambda : pd.DataFrame())
    ...
    @staticmethod
    def load(url: str | Path, include_playlists: list[str] = []) -> 'XMLSource':
        input_url = Path(url)
        ...
        return XMLSource(time = date, songs = pd.DataFrame(songs), playlists = playlists)
    
    def is_in_playlist(self, playlist_name: str) -> pd.Series[bool]:
